...
def plotvoronoiOutput( data_points, vor , CellRad, nRuns ,     outfilename , OPATH_fig , method , nb  , w , h , OffsetAU , inputimg  , VizbpFile ):
...
def plotvoronoiOverlay( vor , data_points,  CellRad , nRuns ,  outfilename , OPATH_fig ,  nb  , w , h , OffsetAU ,  inputimg    ,  method , VizbpFile ):
	...
	if method ==4:	
		...
		plt.imshow( inputimg , origin = 'lower' )
		plt.plot( datapoints[:,0], datapoints[:, 1], 'go', ms=2)
		...
		for vpair in vor.ridge_vertices:
			if vpair[0] >= 0 and vpair[1] >= 0:
				v0 = vor.vertices[vpair[0]]
				v1 = vor.vertices[vpair[1]]
				plt.plot([v0[0], v1[0]], [v0[1], v1[1]], 'y', linewidth = 1 )
...
def plotvoronoiRaw( vor ,  CellRad , nRuns ,  inner_bp , outfilename , OPATH_fig ,  w , h , OffsetAU ,  inputimg  , method  ):
	...
	if method ==4:
		...
		vorpoints    = np.array( vor.points )
		datapoints  = vorpoints #np.array( data_points )
		#hull            = ConvexHull( points  )
		#points      = np.array( points + inner_bp )
		#hull_xpts   =  np.hstack( ( points[hull.vertices,0] , points[hull.vertices[0],0] ) )
		#hull_ypts   =  np.hstack( ( points[hull.vertices,1] , points[hull.vertices[0],1] ) )
